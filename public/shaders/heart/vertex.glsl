...
void main()
{
    mat4 c = inverse(u_camera);
    mat4 r = rotate(u_rotation.x, u_rotation.y, u_rotation.z);
    mat4 s = scale(u_size, u_size, u_size);

    v_normal = a_normal * mat3(r * u_position);

    gl_Position = vec4(a_shape, 1.0) * s * u_position * r * c * u_perspective;
}
